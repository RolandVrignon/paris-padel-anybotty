...
import { FOURPADEL_CLUBS } from './fourpadel-booking.js'
import { walletRequirement } from './scheduled-wallet.js'
...
const localInstant = (wall, zone) => {
  const base = dayjs.utc(wall)
  const matches = [0, 1, 2, 4].map(offset => base.subtract(offset, 'hour'))
    .filter(time => time.tz(zone).format('YYYY-MM-DDTHH:mm') === wall)
  if (matches.length !== 1) throw new Error('Club local time is ambiguous or nonexistent; supply an explicit timestamp with offset')
  return matches[0]
}

export const calculateSchedule = (input, catalog, { now = new Date() } = {}) => {
  if (!input || Object.keys(input).some(key => !['request', 'opening', 'mode', 'provider'].includes(key))) throw new Error('Only request, opening, mode and provider are accepted')
  const mode = input.mode ?? 'preview'
  if (!['preview', 'pay'].includes(mode)) throw new Error('mode must be preview or pay')
  const provider = input.provider ?? 'anybuddy'
  if (!['anybuddy', '4padel'].includes(provider)) throw new Error('Scheduled provider must be anybuddy or 4padel')
  const request = validateRequest(input.request, provider === '4padel' ? Object.keys(FOURPADEL_CLUBS).map(id => ({ id })) : catalog)
  if (request.clubs.length !== 1) throw new Error('Schedule one strategy-approved club per job')
  const zone = provider === '4padel' ? FOURPADEL_CLUBS[request.clubs[0]].timeZone : 'Europe/Paris'
  const wallet = walletRequirement(provider, mode, request)
  ...
  const match = localInstant(`${request.date}T${request.startTime}`, zone)
  let opening
  if (rule.mode === 'explicit') {
    ...
    opening = localInstant(`${date.format('YYYY-MM-DD')}T${rule.localTime}`, zone)
  }
  ...
  return { status: 'ready', mode, provider, walletRequirement: wallet, openingAt: opening.toISOString(), openingParis: opening.tz('Europe/Paris').format(), openingLocal: opening.tz(zone).format(), timeZone: zone, opening: rule, request: { ...request, date: dayjs(request.date).format('DD/MM/YYYY') } }
}
