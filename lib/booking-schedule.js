...
export const calculateSchedule = (input, catalog, { now = new Date() } = {}) => {
  if (!input || Object.keys(input).some(key => !['request', 'opening'].includes(key))) throw new Error('Only request and opening are accepted')
  ...
  return { status: 'ready', openingAt: opening.toISOString(), openingParis: opening.tz(zone).format(), timeZone: zone, opening: rule, request: { ...request, date: dayjs(request.date).format('DD/MM/YYYY') } }
}
