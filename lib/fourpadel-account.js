...
import { fourPadelLocalTime } from './fourpadel-clubs.js'
...
export const normalizeFourPadelReservations = (rows, accountScope) => {
  ...
  return rows.map(row => {
    ...
    return { id: String(row.id), centerId: row.center.id, club: row.center.centerName, court: row.field.name, dateTime: fourPadelLocalTime(row.startingDateZuluTime, row.center.id).slice(0, 16), durationMinutes: row.duration, totalEUR: row.price, status: row.booking_status, reservationConfirmed: row.booking_status === 'Confirmed', paidParts: row.nbOfPaidParticipations ?? null, capacity: row.capacity, fullyPaid: row.paid === true }
  }).filter(Boolean)
}
