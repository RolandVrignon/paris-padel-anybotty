...
export const cancelReservation = async ({ id, read, openDialog, submit, expectedVersion, confirm = false, beforeSubmit }) => {
  ...
  try {
    ...
    try {
      const current = (await read()).find(item => item.id === id)
