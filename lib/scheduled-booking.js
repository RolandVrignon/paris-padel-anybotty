  ...
  return {
    ...
    prepare(input, catalog, options) {
      ...
      return locked(() => {
        ...
        const record = { ...plan, id, status: 'prepared', createdAt: new Date().toISOString(), cronJobId: null, script }
        save(record)
        try {
    ...
    async run(id, execute, { now = new Date() } = {}) {
      ...
      let result
      try {
        result = await execute(claimed.request, claimed.mode)
      ...
      return locked(() => save({ ...claimed, status: result.status, result, paymentSubmitted: result.paymentSubmitted ?? null, reservationConfirmed: result.reservationConfirmed === true, finishedAt: new Date().toISOString() }))
    },
  }
}
