...
import { fourPadelLocalTime, applyFourPadelTimeZone } from './fourpadel-clubs.js'
...
export const inspectFourPadelCancellation = (text, reservation, now = new Date()) => {
  ...
  if (fourPadelLocalTime(now, reservation.centerId).slice(0, 16) >= cutoff) throw new Error('4PADEL cancellation deadline has passed')
...
export const cancelFourPadel = async (session, store, id, { confirm = false, expectedVersion } = {}) => {
  ...
  try {
    await applyFourPadelTimeZone(page, reservation.centerId)
