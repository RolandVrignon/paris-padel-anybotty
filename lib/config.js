...
export const loadFixedConfig = ({ root = repositoryDirectory, env = process.env } = {}) => {
  ...
  if (fixed.providers !== undefined) {
    checkKeys(fixed.providers, ['4padel'], 'providers')
    if (fixed.providers['4padel'] !== undefined) {
      checkKeys(fixed.providers['4padel'], ['account'], 'providers.4padel')
      checkKeys(fixed.providers['4padel'].account, ['email', 'password'], 'providers.4padel.account')
      for (const key of ['email', 'password']) if (typeof fixed.providers['4padel'].account[key] !== 'string') throw new Error(`providers.4padel.account.${key} must be a string`)
    }
  }
