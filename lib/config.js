import { normalizePriceConfig, validatePriceLimit } from './booking-price.js'
...
export const REQUEST_KEYS = ['date', 'startTime', 'durationsMinutes', 'clubs', 'maxTotalPriceEUR', 'maxPricePerHourEUR', 'courtEnvironment']
export const FIXED_KEYS = ['account', 'browser', 'payment', 'providers']
export const PAYMENT_KEYS = ['cardholderName', 'cardNumber', 'expiryMonth', 'expiryYear', 'cvc', 'billingCountry', 'billingPostalCode']
...
export const loadRequestConfig = ({ root = repositoryDirectory, env = process.env, path } = {}) => {
  ...
  return normalizePriceConfig(request)
}
export const loadFixedConfig = ({ root = repositoryDirectory, env = process.env } = {}) => {
  ...
  if (fixed.providers !== undefined) {
    checkKeys(fixed.providers, ['4padel', 'ucpa', 'playtomic'], 'providers')
    for (const provider of ['4padel', 'ucpa', 'playtomic']) {
      if (fixed.providers[provider] === undefined) continue
      checkKeys(fixed.providers[provider], ['account'], `providers.${provider}`)
      checkKeys(fixed.providers[provider].account, ['email', 'password'], `providers.${provider}.account`)
      for (const key of ['email', 'password']) if (typeof fixed.providers[provider].account[key] !== 'string') throw new Error(`providers.${provider}.account.${key} must be a string`)
    }
  }
  if (fixed.payment !== undefined) {
    checkKeys(fixed.payment, PAYMENT_KEYS, 'payment')
    for (const key of PAYMENT_KEYS) {
      if (fixed.payment[key] !== undefined && typeof fixed.payment[key] !== 'string') throw new Error(`payment.${key} must be a string`)
    }
  }
  if (fixed.browser) {
...
export const resolveMaxPricePerHourEUR = (override, { root = repositoryDirectory, env = process.env } = {}) => {
  if (override !== undefined) return validatePriceLimit(override)
  const configured = env.ANYBOTTY_REQUEST_CONFIG_PATH || existsSync(resolve(root, 'config.request.json')) || existsSync(resolve(root, 'config.json'))
  return validatePriceLimit(configured ? loadRequestConfig({ root, env }).maxPricePerHourEUR : undefined)
}
