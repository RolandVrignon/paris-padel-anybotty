...
export const inspectCheckout = async (page, clubId, { cartTimeoutMs = 30000 } = {}) => {
  ...
  try {
    await Promise.all([
      sheet.getByRole('button', { name: /^Payer / }).waitFor({ timeout: cartTimeoutMs }),
      sheet.filter({ hasText: /Total à payer\s*[\d\s,.]+€/ }).waitFor({ timeout: cartTimeoutMs }),
    ])
  } catch (error) {
    if (error.name !== 'TimeoutError') throw error
    throw Object.assign(new Error('Checkout cart did not finish loading; no payment requested'), { code: 'CART_NOT_READY' })
  }
