...
export const monitoringPlan = (target, previous, now = new Date()) => {
  ...
  // Reference calendars expose dates beyond their assumed booking gate.
  ...
  if (!verifying.length && clock - Date.parse(previous.snapshot?.startedAt) < policy.discoveryMinutes * 60000 - 1000) return { mode: 'skip', reason: 'minimum_interval', pattern }
