...
import { DEFAULT_UCPA_CENTER, ucpaAccountUrl, ucpaIdentityUrl } from './ucpa-centers.js'

export const UCPA_ACCOUNT_URL = ucpaAccountUrl(DEFAULT_UCPA_CENTER)
export const UCPA_IDENTITY_URL = ucpaIdentityUrl(DEFAULT_UCPA_CENTER)

export const readUcpaIdentity = async (context, timeoutMs = 20000, center = DEFAULT_UCPA_CENTER) => {
  let response
  try {
    response = await context.request.get(ucpaIdentityUrl(center), { maxRedirects: 0, timeout: timeoutMs })
  } catch (error) {
