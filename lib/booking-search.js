  ...
  for (const clubId of request.clubs) {
    ...
    for (let count = 0; count < maxAttemptsPerClub; count++) {
      ...
      try {
        ...
        // A loading/failed cart is not a sold-out court. Retry its preparation
        // once, on a fresh page, without blacklisting the still-available offer.
        if (error.code === 'CART_NOT_READY') {
          record({ clubId, status: 'cart_not_ready', retrying: cartRetries === 0 })
          if (cartRetries++ === 0 && count + 1 < maxAttemptsPerClub) continue
          return { status: 'incomplete', reason: 'cart_not_ready', events, paymentSubmitted: false, reservationConfirmed: false }
        }
