...
export const ucpaBookingKey = (request, center = DEFAULT_UCPA_CENTER) => `${center.id === DEFAULT_UCPA_CENTER.id ? '' : `${center.id}-`}book-${request.date}-${request.startTime.replace(':', '')}`
...
export const reconcileUcpaBooking = async (client, store, request, { center: selectedCenter = client.center || DEFAULT_UCPA_CENTER } = {}) => {
  const center = resolveUcpaCenter(selectedCenter.id || selectedCenter)
  const key = ucpaBookingKey(request, center)
...
export const bookUcpa = async (session, client, store, input, { confirm = false, onStage = () => {}, preview = previewUcpaBooking, center: selectedCenter = client.center || DEFAULT_UCPA_CENTER } = {}) => {
  const center = resolveUcpaCenter(selectedCenter.id || selectedCenter)
  const request = normalizeUcpaRequest(input)
  const key = ucpaBookingKey(request, center)
  const previous = store.read(key)
  if (confirm && previous) return reconcileUcpaBooking(client, store, request, { center })
  ...
  const guard = await installUcpaPreviewGuard(session.context, { center })
  try {
    const offer = await preview(page, request, { guard, onStage, center })
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        const result = await reconcileUcpaBooking(client, store, request, { center })
...
export const cancelUcpa = async (session, client, store, id, { confirm = false, expectedVersion, retry = false, center: selectedCenter = client.center || DEFAULT_UCPA_CENTER } = {}) => {
  const center = resolveUcpaCenter(selectedCenter.id || selectedCenter)
  if (!validUcpaId(id)) throw new Error('Use a UCPA reservation ID returned by list')
  const key = `cancel-${id}`
  const previous = store.read(key)
  ...
  // An explicit retry still requires an active, freely cancellable reservation
  // and a fresh preview. Preserve the earlier attempt instead of deleting it.
  const retrying = retry && previous?.status === 'cancellation_unverified' && previous.responseConfirmed === false
  if (previous && previous.status !== 'cancellation_preview' && !retrying) return { provider: 'ucpa', status: 'cancellation_unverified', submissionStarted: true, verified: false }
  ...
  try {
    ...
    const journal = { provider: 'ucpa', status: 'cancellation_started', reservation, terms, feeEUR: 0, version, startedAt: new Date().toISOString(), ...(retrying ? { previousAttempt: previous } : {}) }
    const cancelUrl = client.cancelUrl || `${ucpaAccountApi(center)}/cancel-court-session`
    const pending = waitMutation(page, cancelUrl)
    let responseConfirmed = false
    try {
      await clickUcpaCancellation(page, async () => {
        store.save(key, journal)
        // The native external-session cancellation uses the Horanet contact ID.
        guard.arm(cancelUrl, payload => String(payload?.sessionId) === id && /^horanet_id_[a-zA-Z0-9_-]+$/.test(client.contactId || '') && payload?.uuid === client.contactId)
      })
      responseConfirmed = await successResponse(pending)
