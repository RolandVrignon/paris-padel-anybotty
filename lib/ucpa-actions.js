...
export const cancelUcpa = async (session, client, store, id, { confirm = false, expectedVersion, retry = false } = {}) => {
  if (!validUcpaId(id)) throw new Error('Use a UCPA reservation ID returned by list')
  const key = `cancel-${id}`
  const previous = store.read(key)
  const rows = await client.list()
  ...
  try {
    ...
    try {
      await clickUcpaCancellation(page, async () => {
        ...
        guard.arm(UCPA_CANCEL_URL, payload => String(payload?.sessionId) === id && /^horanet_id_[a-zA-Z0-9_-]+$/.test(client.contactId || '') && payload?.uuid === client.contactId)
      })
      responseConfirmed = await successResponse(pending)
