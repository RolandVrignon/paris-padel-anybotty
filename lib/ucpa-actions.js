...
export const bookUcpa = async (session, client, store, input, { confirm = false, onStage = () => {}, preview = previewUcpaBooking } = {}) => {
  ...
  try {
    ...
    if (!await page.getByText('Carte enregistrée', { exact: true }).isVisible() || !await page.getByText(/^Carte Bancaire\s+[Xx*•\s]+\d{4}$/i).isVisible()) throw new Error('UCPA saved card is required; card entry is not implemented')
