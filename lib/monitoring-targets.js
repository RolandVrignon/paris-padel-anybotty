...
export const successfulObservation = (target, previous, snapshot) => {
  ...
  const lastFullScan = snapshot.scanMode !== 'targeted' ? { at: snapshot.finishedAt, frontier: snapshot.navigationThroughDate || snapshot.publishedDates?.at(-1) || snapshot.slots.at(-1)?.startDateTime.slice(0, 10), window: snapshot.window, horizon: horizonSummary(snapshot) } : baseline.lastFullScan
