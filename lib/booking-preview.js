...
import { assertCheckoutEnvironment, checkoutCourtLocator } from './court-environment.js'
import { selectCourtIfOffered, NoMatchingOfferError, offerKey } from './court-selection.js'
...
export const previewBookingOffer = async (page, club, request, { excludedOffers = [], onSelected = () => {}, onStage = () => {} } = {}) => {
  ...
  const courtName = checkoutCourtLocator(sheet)
  await courtName.waitFor()
  const actualCourt = (await courtName.innerText()).trim()
  if (!actualCourt || (selected.court && selected.court !== actualCourt)) throw new Error('Selected court does not match the checkout')
  if (!selected.court) {
    selected.court = actualCourt
