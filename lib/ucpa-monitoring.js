...
export const assembleUcpaSnapshot = (target, window, weeks, startedAt) => {
  ...
  if (weeks[0].dates[0] > window.from || navigationThroughDate < window.from) throw new Error('UCPA calendar date changed')
  ...
  const monitoredWindow = { from: window.from, to: boundary > window.to ? boundary : window.to }
  return {
    startedAt, finishedAt: new Date().toISOString(), window: monitoredWindow,
    slots: weeks.flatMap(week => week.slots).filter(slot => slot.startDateTime.slice(0, 10) >= window.from),
    publishedDates: weeks.flatMap(week => week.publishedDates).filter(date => date >= window.from),
...
export const fetchUcpaAvailability = async (target, window, { signal } = {}) => {
  ...
  try {
    ...
    for (let index = 0; index < 12; index++) {
      ...
      if (classes.includes('pointer-events-none') && classes.includes('cursor-not-allowed')) return assembleUcpaSnapshot(target, window, weeks, startedAt)
