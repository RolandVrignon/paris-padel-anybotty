...
export const reconcilePayment = async (journal, readReservations) => {
  ...
  return { status: journal.status === 'payment_failed' ? 'payment_failed' : journal.status === 'payment_action_required' ? 'payment_action_required' : 'payment_unverified', stage: 'payment', paymentSubmitted: true, reservationConfirmed: false, ...(matches.length === 1 ? { reservation: matches[0] } : {}) }
}

export const inspectFinalPayment = async (page, offer, request) => {
  ...
  if (parseEURCents(label.replace(/^Payer\s+/, '').replace(/\s*€$/, '')) !== offer.totalCents || !await button.isEnabled()) throw new Error('Final payment button amount changed or disabled')
...
export const payBookingOffer = async (page, offer, request, { payment, store, readReservations, headed = false, timeoutMs = headed ? 180000 : 60000, pollMs = 2000, prepare = prepareStripeCheckout, fill = fillStripeCard, inspect = inspectFinalPayment, click = clickFinalPayment, onEvent = () => {} }) => {
  ...
  const button = await inspect(page, offer, request)
  ...
  try {
    ...
    do {
      ...
      if (!mismatch && last.reservationConfirmed) { store.save({ ...journal, ...last, stripeStatus }); return { ...offer, ...last, stripeStatus } }
      if (stripeStatus === 'requires_action' && !actionReported) { actionReported = true; onEvent({ status: 'payment_action_required', message: headed ? 'Valider la demande bancaire dans le navigateur ouvert.' : 'Validation bancaire requise ; ne pas relancer le paiement.' }) }
      if (mismatch || ['requires_payment_method', 'canceled'].includes(stripeStatus) || (stripeStatus === 'requires_action' && !headed)) break
      await pause(pollMs)
    ...
    const result = { ...offer, status, stage: 'payment', stripeStatus, paymentSubmitted: true, reservationConfirmed: false }
