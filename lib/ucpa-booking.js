...
export const UCPA_PADEL_URL = 'https://www.ucpa.com/sport-station/paris-19/mon-terrain-padel'
// UCPA Paris 19: all four courts are indoor (confirmed by the user).
const COURT_ENVIRONMENT = 'indoor'
const acceptsUcpaEnvironment = value => validateCourtEnvironment(value).some(environment => environment === 'any' || environment === COURT_ENVIRONMENT)
...
export const ucpaPreviewRequestAllowed = (url, method) => {
  if (['GET', 'HEAD', 'OPTIONS'].includes(method)) return true
  const target = new URL(url)
  return method === 'POST' && target.origin === ORIGIN && (READ_POSTS.has(target.pathname) || ['/sport-station/espacepersonnel/api/paris-19/user/participants/allowedProduct', '/sport-station/espacepersonnel/api/paris-19/amplify/kala/reservedSession'].includes(target.pathname))
}
export const installUcpaPreviewGuard = async context => {
  let mutation = null
  let consumed = false
  const handler = route => {
    const req = route.request()
    if (ucpaPreviewRequestAllowed(req.url(), req.method())) return route.fallback()
  ...
  return {
    ...
    arm: (url, accept) => {
      if (mutation || !['https://www.ucpa.com/loisirs-reservation/api/users/createCourtBooking', 'https://www.ucpa.com/sport-station/espacepersonnel/api/paris-19/cancel-court-session'].includes(url) || typeof accept !== 'function') throw new Error('Invalid UCPA mutation authorization')
      mutation = { url, accept }
    },
...
export const inspectUcpaSummary = (text, request, durationMinutes, court) => {
  ...
  if (!lines.some(line => line.toLocaleLowerCase('fr') === frenchDate(request.date)) || !lines.includes(timeRange(request.startTime, durationMinutes)) || !lines.includes(court) || !lines.includes('Padel')) throw new Error('UCPA summary date, time, court or sport differs from the selected offer')
...
const assertSelection = async (page, request, duration, court) => {
  const date = page.getByText(new RegExp(`^${frenchDate(request.date)}$`, 'i'))
  await date.waitFor()
...
export const advanceUcpaFunnel = async (page, request, durationMinutes, { verifyIdentity = async () => {}, onStage = () => {} } = {}) => {
  ...
  return { provider: 'ucpa', clubId: 'ucpa-paris', status: pricing.withinBudget ? 'checkout_ready' : 'no_match', ...(!pricing.withinBudget ? { reason: 'price_limit' } : {}), date: request.date, startTime: request.startTime, durationMinutes, court, environment: COURT_ENVIRONMENT, ...pricing, termsAccepted: false, reservationSubmitted: false, paymentSubmitted: false }
}

export const previewUcpaBooking = async (page, input, { onStage = () => {}, guard } = {}) => {
  const request = normalizeUcpaRequest(input)
  if (!acceptsUcpaEnvironment(request.courtEnvironment)) return { provider: 'ucpa', clubId: 'ucpa-paris', status: 'no_match', reason: 'environment_excluded', environment: COURT_ENVIRONMENT, date: request.date, startTime: request.startTime, reservationSubmitted: false, paymentSubmitted: false }
  ...
  const expectedIdentity = await readUcpaIdentity(page.context())
  if (!expectedIdentity) throw new Error('UCPA session expired')
  if (!guard) await installUcpaPreviewGuard(page.context())
  ...
  await page.goto(UCPA_PADEL_URL, { waitUntil: 'domcontentloaded' })
  ...
  for (let index = 0; index < 12; index++) {
    ...
    if (week.dates.includes(request.date)) {
      ...
      const column = component.locator('.mobile-day').filter({ has: page.getByRole('heading', { name: dayName, exact: true }) }).filter({ has: page.getByText(String(Number(request.date.slice(-2))), { exact: true }) })
      const range = timeRange(request.startTime, slot.durationMinutes).replace(' à ', ' - ').replaceAll(':', 'h')
      const row = column.locator('li').filter({ hasText: range })
      ...
      await row.getByRole('button', { name: /RÉSERVER$/ }).click()
      const result = await advanceUcpaFunnel(page, request, slot.durationMinutes, { onStage, verifyIdentity: async () => {
