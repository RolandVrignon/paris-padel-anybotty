...
export const validateRequest = (input, catalog) => {
  ...
  const allowed = new Set(['date', 'startTime', 'durationsMinutes', 'clubs', 'maxTotalPriceEUR'])
  ...
  if (!Array.isArray(input.durationsMinutes) || !input.durationsMinutes.length || input.durationsMinutes.some(value => ![60, 90].includes(value))) throw new Error('durationsMinutes accepts 60, 90 or both')
  ...
  return { ...input, date: parsed.format('YYYY-MM-DD'), durationsMinutes: [...new Set(input.durationsMinutes)] }
}
...
export const buildPlan = (input, catalog, { now = new Date() } = {}) => {
  ...
  return {
    ...
    warning: 'Opening dates are estimates from last available dates, not verified booking rules. Availability and accepted durations have not been checked.',
