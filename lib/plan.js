...
export const validateRequest = (input, catalog) => {
  ...
  const allowed = new Set(['date', 'startTime', 'durationsMinutes', 'clubs', 'maxTotalPriceEUR', 'maxPricePerHourEUR', 'courtEnvironment'])
  ...
  const normalized = normalizePriceConfig(input)
  return { ...normalized, maxPricePerHourEUR: validatePriceLimit(normalized.maxPricePerHourEUR), courtEnvironment: validateCourtEnvironment(input.courtEnvironment), date: parsed.format('YYYY-MM-DD'), durationsMinutes }
}
