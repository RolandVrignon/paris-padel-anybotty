...
export const updateCalendar = (club, previous, snapshot) => {
  ...
  batches = batches.filter(batch => Date.parse(batch.firstSeenAt) >= cutoff).slice(-200).map(batch => {
  ...
  return { calendar: { window: snapshot.window, watches, batches, additionalSlots }, completedWatches, openingWatch }
}
