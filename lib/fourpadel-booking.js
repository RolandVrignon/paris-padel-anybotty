...
export const FOURPADEL_CLUBS = {
  '4padel-paris-20': { centerId: 79, name: '4PADEL Paris 20' },
  '4padel-saint-ouen': { centerId: 117, name: '4PADEL Saint-Ouen' },
  '4padel-boulogne': { centerId: 105, name: '4PADEL Boulogne-Billancourt' },
  '4padel-saint-louis-bale': { centerId: 61, name: '4PADEL Saint-Louis - Bâle' },
}
...
export const normalizeFourPadelRequest = input => {
  ...
  if (!Object.hasOwn(FOURPADEL_CLUBS, input.clubId || '')) throw new Error('Choose a supported official 4PADEL club')
...
export const selectFourPadelOffer = (rows, input) => {
  ...
  for (const durationMinutes of request.durationsMinutes) {
    const candidates = rows.filter(row => parisTime(row.startingDateZuluTime).slice(0, 16) === `${request.date}T${request.startTime}` && row.duration === durationMinutes).flatMap(row => {
...
export const previewFourPadelBooking = async (page, input, { onStage = () => {}, accountScope, parts = 1 } = {}) => {
  ...
  const today = parisTime(new Date()).slice(0, 10)
  if (`${request.date}T${request.startTime}` <= parisTime(new Date()).slice(0, 16)) throw new Error('4PADEL slot is in the past')
  ...
  try {
    ...
    visibility.catch(() => {})
    const initialCalendar = page.waitForResponse(r => new URL(r.url()).origin === 'https://api2-front.lefive.fr' && new URL(r.url()).pathname === '/bookingrules/allFields')
    initialCalendar.catch(() => {})
    await page.goto(`${ORIGIN}/reservations/slots?center=${FOURPADEL_CLUBS[request.clubId].centerId}`, { waitUntil: 'domcontentloaded' })
    ...
    const calendarResponse = await initialCalendar
    ...
    // The portal can restore a different club from local storage despite ?center=.
    const club = FOURPADEL_CLUBS[request.clubId]
    const selector = page.getByRole('button', { name: /Club\s+4PADEL/ })
    if (!(await selector.innerText()).includes(club.name)) {
      await selector.click()
      const changed = page.waitForResponse(r => new URL(r.url()).origin === 'https://api2-front.lefive.fr' && new URL(r.url()).pathname === '/bookingrules/allFields')
      changed.catch(() => {})
      await page.getByText(club.name, { exact: true }).click()
      if (!(await changed).ok()) throw new Error('4PADEL club calendar unavailable')
    }
    ...
    const day = new Intl.DateTimeFormat('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', timeZone: 'Europe/Paris' }).format(new Date(`${request.date}T12:00:00Z`))
