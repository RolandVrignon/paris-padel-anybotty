...
export const providerSessionFiles = (root, provider) => {
  if (!['ucpa', '4padel', 'playtomic'].includes(provider)) throw new Error('Unsupported authentication provider')
...
export const createProviderSession = async (provider, authenticate, {
  ...
  try {
    ...
    if (signal?.aborted) throw new ProviderAuthError(provider, 'aborted', 'authentication interrupted')
    const context = await browser.newContext({ ...(stored ? { storageState: stored } : {}), ...(provider === '4padel' ? {} : { timezoneId: 'Europe/Paris' }), serviceWorkers: 'block' })
