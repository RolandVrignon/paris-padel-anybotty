...
export const failureRecord = (club, previous, error, now = new Date()) => {
  ...
  const accessBlocked = [401, 403, 429].includes(error.httpStatus)
  const authenticationFailure = ['login_required', 'login_refused', 'authentication_unavailable', 'identity_unavailable'].includes(error.code)
  const monitoringAlert = accessBlocked || (authenticationFailure && failures >= 2) ? { type: accessBlocked ? 'access_restricted' : 'repeated_authentication_failure', provider: club.provider, detectedAt: now.toISOString(), action: 'monitoring_paused' } : null
  const delay = Math.max(monitoringAlert ? 6 * 3600000 : 0, Math.min(300000 * 2 ** Math.min(failures - 1, 4), 3600000), error.retryAfterMs || 0)
  return { ...previous, version: 1, clubId: club.id, name: club.name, status: 'error', attemptedAt: now.toISOString(), attemptedAtParis: parisTime(now), failures, nextRetryAt: new Date(now.getTime() + delay).toISOString(), error: error.message, httpStatus: error.httpStatus ?? null, monitoringAlert, events: [] }
}
