# Guide complet — Paris Padel / Anybotty
...
| Fonction | État |
| --- | --- |
| Catalogue de onze clubs et planning prévisionnel | Disponible |
| Surveillance des ouvertures sur seize canaux, avec fréquence adaptative | Disponible via le timer systemd |
...
Ces commandes effectuent **un vrai paiement**. Le moteur essaie les possibilités dans l’ordre club → durée → intérieur/extérieur → terrain. Au premier récapitulatif conforme, il accepte les conditions connues, revérifie le club, la date, l’heure, le terrain, la durée et le total avant d’ouvrir Stripe, puis sélectionne Carte bancaire et remplit les champs privés. Le terrain est relevé même sans modale. Comme Stripe masque ensuite le récapitulatif, le contrôle final porte sur le montant exact du bouton Payer et la continuité de la page. Le plafond `maxPricePerHourEUR` s’applique toujours ; un changement du total depuis le récapitulatif bloque le paiement.

Le bouton final est identifié dans le checkout par son libellé exact et son montant. Son clic DOM évite le déplacement vers Revolut Pay constaté pendant le test UCPA. Une seule requête de confirmation Stripe est autorisée ; les autres confirmations restent bloquées. Les régressions automatisées s’exécutent sur des fixtures locales, sans nouveau paiement. Un contrôle réel à UCPA a aussi validé le récapitulatif, le remplissage de la carte et le bouton final à 38 €, avec toute confirmation réseau bloquée.
...
Pour Anybuddy, le scan complet horaire interroge les sept clubs actifs successivement, sur une fenêtre d’au moins **J à J+35 inclus**, étendue si nécessaire jusqu’à l’horizon observé + 14 jours. Toutes les durées et tous les terrains renvoyés par ce calendrier sont conservés. Les relevés officiels suivent leur propre calendrier et restent séparés par fournisseur.
...
| [padel-booking](../skills/padel-booking/SKILL.md) | `/padel_booking` | Lire/modifier les préférences, simuler, réserver et suivre le paiement |
...
La lecture a été validée sur le compte réel. Le parcours UCPA du 19 septembre 2026 à 07 h a été payé puis annulé lors du test du 11 septembre ; le statut annulé a été vérifié. Les tests de régression utilisent des données simulées. Le transport de lecture suit l'action serveur du site Anybuddy, découverte dans ses fichiers JavaScript actuels ; ce n'est pas une API publique stable. Un changement de structure provoque une erreur explicite.
