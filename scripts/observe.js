...
import { calendarWindow, updateCalendar, calendarSummary } from '../lib/calendar-monitor.js'
  ...
  if (!release) {
    ...
    try {
      ...
      const outcomes = await Promise.allSettled((globalPause ? [] : clubs).map(async club => {
        ...
        let record
        try {
          const snapshot = await fetchAvailability(club, calendarWindow(club), { signal: controller.signal })
          record = { ...nextRecord(club, previous, snapshot), ...updateCalendar(club, previous, snapshot) }
        } catch (error) {
          if (controller.signal.aborted) return
