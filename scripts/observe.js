...
import { fetchAvailability, queryWindow, nextRecord, failureRecord } from '../lib/availability.js'
...
if (args[0] === '--report') {
  ...
  if (!release) {
    ...
    try {
      ...
      for (const club of globalPause ? [] : clubs) {
        ...
        let record
        try {
          const snapshot = await fetchAvailability(club, queryWindow(new Date()), { signal: controller.signal })
          record = nextRecord(club, previous, snapshot)
        } catch (error) {
          if (controller.signal.aborted) break
        ...
        // Rate limiting or denied access can affect the whole host. Stop this
        // pass rather than continuing against other clubs on the same server.
        if ([401, 403, 429].includes(record.httpStatus)) break
