...
try {
  ...
  if (!club || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^([01]\d|2[0-3]):[0-5]\d$/.test(time)) throw new Error('Usage: checkout-preview.js --club ID --date YYYY-MM-DD --time HH:mm [--duration 60|90|120 | --durations 60,90,120] [--court NAME] [--court-environment indoor,outdoor|outdoor,indoor|indoor|outdoor|any] [--max-price-per-hour EUR] [--to-stripe] [--headless]')
