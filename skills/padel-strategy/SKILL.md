...
**Attendre est une décision, pas une tâche programmée.** Le timer existant surveille les ouvertures mais ne déclenche pas de recherche de réservation. Ne pas promettre de revenir automatiquement, de notifier ou de réserver à une heure donnée sans mécanisme effectivement configuré et vérifié. Pour cette version, indiquer que le prochain essai reste à lancer ; ne pas créer de nouvelle automatisation à partir d’une simple analyse de stratégie. Une demande explicite de programmation nécessite de traiter cette étape séparément.
